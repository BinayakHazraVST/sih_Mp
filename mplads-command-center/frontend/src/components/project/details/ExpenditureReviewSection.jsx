            ...
            {categories.map((cat, idx) => (
              <div key={idx} className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1.5 hover:bg-slate-100/70 transition">
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-black text-slate-900">{formatCurrency(cat.amount, true)}</span>
                    <span className="text-[11px] font-bold text-slate-500">({cat.percentage}%)</span>
                    <Badge variant={cat.status === 'VERIFIED' ? 'emerald' : 'amber'} className="text-[9px] px-1.5 py-0">
                      {cat.status === 'VERIFIED' ? 'Verified' : 'Variance'}
                    </Badge>
                  </div>
                </div>
                ...
                <div className="w-full bg-slate-200/70 rounded-full h-1.5 overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      cat.status === 'VERIFIED' ? 'bg-emerald-500' : 'bg-amber-500'
                    }`}
          ...
          <div className="p-4 bg-amber-50/70 border border-amber-200 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-amber-900/90 font-medium mt-0.5 leading-relaxed">
            ...
            {varianceItem && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl text-xs font-bold transition shrink-0 cursor-pointer shadow-2xs"
              >
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <span className="text-slate-400 block font-semibold text-[10px] uppercase">Claimed Rate</span>
                  <strong className="text-slate-900 text-sm font-black mt-0.5 block">{formatCurrency(varianceItem.claimedAmount, true)}</strong>
                </div>

                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <span className="text-slate-400 block font-semibold text-[10px] uppercase">Benchmark</span>
                  <strong className="text-emerald-700 text-sm font-black mt-0.5 block">{formatCurrency(varianceItem.benchmarkAmount, true)}</strong>
                </div>

                <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl">
                  <span className="text-amber-700 block font-semibold text-[10px] uppercase">Difference</span>
                  <strong className="text-amber-900 text-sm font-black mt-0.5 block">+{varianceItem.variancePercentage}%</strong>
                </div>
              </div>

              <div className="p-3 bg-indigo-50/60 border border-indigo-100 rounded-xl text-indigo-950 space-y-1">
                <span className="font-extrabold block text-indigo-900">AI Analysis Observation:</span>
                <p className="font-medium leading-relaxed">{varianceItem.explanation}</p>
              </div>

              <div className="p-3 bg-emerald-50/60 border border-emerald-100 rounded-xl text-emerald-950 space-y-1">
                <span className="font-extrabold block text-emerald-900">Recommended Action for MP:</span>
                <p className="font-medium leading-relaxed">{varianceItem.recommendation}</p>
              </div>
            </div>
