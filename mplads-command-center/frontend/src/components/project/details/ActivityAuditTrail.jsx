...
const ACTIVITY_ICON_MAP = {
  ...
  inspection: { icon: ClipboardCheck, bg: 'bg-violet-50 text-violet-600 border-violet-200' },
  progress: { icon: CheckCircle2, bg: 'bg-amber-50 text-amber-600 border-amber-200' },
  sanction: { icon: CheckCircle2, bg: 'bg-slate-100 text-slate-700 border-slate-300' },
};
            ...
            {activity.map((item, idx) => {
              ...
              return (
                <div key={item.id || idx} className="relative group">
                  ...
                  <div className="p-3 bg-slate-50/70 border border-slate-200/80 rounded-xl space-y-1 hover:bg-slate-50 transition">
                    ...
                    <div className="flex items-center justify-between text-[11px] text-slate-600 pt-0.5">
                      <span className="text-slate-500 font-medium">Actor: <strong className="text-slate-700">{item.actor}</strong></span>
                      {item.details && (
                        <span className="text-indigo-700 font-medium italic truncate max-w-xs">{item.details}</span>
                      )}
                    </div>
                  </div>
                </div>
