      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
          <div className="flex items-start gap-3">
            <div className="w-11 h-11 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-bold shrink-0 shadow-xs">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <Badge variant={contractor.performanceScore >= 80 ? 'emerald' : contractor.performanceScore >= 60 ? 'indigo' : 'amber'}>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white border border-indigo-200 hover:border-indigo-600 rounded-xl text-xs font-bold transition cursor-pointer self-start sm:self-center"
          >
        ...
        {contractor.delaySignal && (
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2.5 text-xs text-amber-900 font-medium">
            ...
            <div>
              <strong className="font-extrabold text-amber-950 block">Contractor Performance Risk Signal:</strong>
              <p className="mt-0.5">{contractor.delaySignal}</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Completed Works</span>
            <strong className="text-base font-black text-emerald-700 mt-1 block">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl">
            ...
            <strong className="text-base font-black text-indigo-700 mt-1 block">
              {contractor.onTimePercentage ?? 88}%
            </strong>
          </div>

          <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Delayed Works</span>
            <strong className={`text-base font-black mt-1 block ${(contractor.delayedProjects || 0) > 0 ? 'text-amber-700' : 'text-slate-700'}`}>
