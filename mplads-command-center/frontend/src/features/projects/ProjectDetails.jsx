          ...
          <Card title="Financial Sanctions & Utilization">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="p-3 bg-slate-900/60 border border-slate-800 rounded-lg">
                ...
                <span className="text-lg font-bold text-slate-100">{formatCurrency(project.sanctionedAmount)}</span>
              </div>
              <div className="p-3 bg-slate-900/60 border border-slate-800 rounded-lg">
                <span className="text-xs text-slate-400 block">Released Amount</span>
                <span className="text-lg font-bold text-cyan-400">{formatCurrency(project.releasedAmount)}</span>
              </div>
              <div className="p-3 bg-slate-900/60 border border-slate-800 rounded-lg">
                <span className="text-xs text-slate-400 block">Utilized Amount</span>
                <span className="text-lg font-bold text-emerald-400">{formatCurrency(project.utilizedAmount)}</span>
              </div>
            </div>
          </Card>
          ...
          <Card title="Contractor & Beneficiary Impact">
            <div className="space-y-3">
              <div className="flex items-center gap-3 text-xs">
                ...
                <span className="text-slate-300">Contractor ID: {project.contractorId || 'CON001'}</span>
              </div>
              <div className="flex items-center gap-3 text-xs">
                <Users className="w-4 h-4 text-emerald-400" />
                <span className="text-slate-300">Target Beneficiaries: {project.beneficiaries?.toLocaleString()} citizens</span>
              </div>
            </div>
          </Card>
        </div>
